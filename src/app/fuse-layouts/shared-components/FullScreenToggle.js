...
const HeaderFullScreenToggle = props => {
	...
	return (
		<Tooltip title="Fullscreen toggle" placement="bottom">
			<IconButton onClick={toggleFullScreen} style={{backgroundColor: '#8BB6DD', marginRight:30}} className={clsx('w-40 h-40', props.className)}>
				<Icon className="text-white">{isFullScreen ? 'fullscreen_exit' : 'fullscreen'}</Icon>
			</IconButton>
		</Tooltip>
	);
};
